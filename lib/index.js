...
class WemoPlatform {
  ...
  discoverDevices () {
    ...
    if (this.discoveryRun % 3 === 2) {
      ...
      setTimeout(() => {
        ...
        if (this.config.debug && existSerialsToConnect.length > 0) {
          const names = accessoryArray.filter(el =>
            existSerialsToConnect.includes(el.context.serialNumber)
          )
          if (names.length > 0) {
            this.log.warn(
              '%s: [%s].',
              this.lang.awaitingRe,
              names.map(el => el.displayName).join('], [')
            )
          }
        }
  ...
  async initialiseDevice (device) {
    try {
      ...
      if (
        device.serviceList &&
        Array.isArray(device.serviceList.service) &&
        device.serviceList.service.length > 0
      ) {
