...
class WemoPlatform {
  ...
  discoverDevices () {
      ...
      ssdpClient.on('response', async (msg, statusCode, rinfo) => {
        ...
        try {
          ...
          if (accessory && accessory.context.initialised) {
            if (accessory.context.controllable || !accessory.client) {
    ...
    // ********************** \\
    // Startup Device Logging \\
    // ********************** \\
    if (this.discoveryRun % 3 === 2 && cacheSerialsToConnect.length > 0) {
  ...
  disableClient (accessory, err) {
    ...
    // Add the device back to the pending list
  ...
  reenableClient (accessory, deviceData) {
    ...
    // Remove the device from the pending connection list
    cacheSerialsToConnect = cacheSerialsToConnect.filter(el => el !== deviceData.serialNumber)
