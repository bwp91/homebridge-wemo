...
class WemoPlatform {
  ...
  pluginSetup () {
    ...
    try {
      ...
      devicesInHB.forEach(accessory => {
        ...
        if (
          ...
          (this.config.mode === 'semi' &&
            !serialsToConnect.includes(accessory.context.serialNumber))
        ) {
      ...
      listenerServer = http.createServer((req, res) => {
        ...
        const accessory = [...devicesInHB.values()].find(el => el.context.udn === udn)
  ...
  discoverDevices () {
    ...
    if (
      ...
      accessoryArray.filter(el => !el.context.controllable).length === 0
    ) {
      return
    }
    ...
    if (
      this.config.mode === 'auto' ||
      (this.config.mode === 'semi' && serialsToConnect.length > 0)
    ) {
        ...
        try {
          ...
          if (accessory && accessory.context.initialised) {
            if (accessory.context.controllable) {
              ...
              this.reenableClient(accessory, deviceData)
            }
          } else {
    ...
    if (this.discoveryRun % 3 === 2) {
      ...
      setTimeout(() => {
        const names = accessoryArray.filter(el => !el.context.controllable)
        if (names.length > 0) {
          this.log.warn(
            '%s: [%s].',
            this.lang.awaiting,
  ...
  async getDeviceInfo (ip, portToTryFirst) {
    ...
    const tryPort = async (port, ipAddress) => {
      try {
        ...
        device.port = port
        device.cbURL =
          'http://' + this.getLocalInterfaceAddress(ipAddress) + ':' + listenerServer.address().port
  ...
  async initialiseDevice (device) {
    try {
      ...
      serialsToConnect = serialsToConnect.filter(el => el !== device.serialNumber)
      ...
      const context = {
        connection: deviceConf.listenerType === 'http' ? 'http' : 'upnp',
        cbURL: device.cbURL,
      ...
      if (device.serviceList && Array.isArray(device.serviceList.service)) {
        ...
        if (context.enableDebugLogging) {
          this.log.warn('[%s] %s.', this.name, this.lang.noUPNP)
        }
      }
      ...
      switch (device.deviceType) {
          ...
          *****************/
          ...
          subdevices.forEach(subdevice => {
            try {
              ...
              subAcc.context.controllable = true
        ...
        case 'urn:Belkin:device:NetCamSensor:1': {
          ...
          ***********/
          ...
          // Wemo motions must use upnp as http is not supported
          if (Object.keys(context.serviceList).length > 0) {
            accessory.client = new (require('./connection/upnp'))(this, accessory)
          }
  ...
  disableClient (accessory, err) {
    ...
    if (this.deviceConf[accessory.context.serialNumber]) {
      serialsToConnect.push(accessory.context.serialNumber)
    }
  ...
  reenableClient (accessory, deviceData) {
    accessory.context = {
      ...accessory.context,
      ...{
    ...
    // Remove the device from the pending connection list
    serialsToConnect = serialsToConnect.filter(el => el !== deviceData.serialNumber)

    // Update the context now the device is controllable again
    accessory.context.controllable = true
    if (accessory.client) {
    ...
    this.log('[%s] connection has been re-established.', accessory.displayName)
  }
