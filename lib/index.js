...
class WemoPlatform {
  ...
  discoverDevices () {
    ...
    if (this.config.mode !== 'auto' && Object.values(devicesToConnect).length === 0) {
      return
    }
    ...
    if (
      ...
      ssdpClient.on('response', async (msg, statusCode, rinfo) => {
        ...
        try {
          ...
          queue.add(() => this.initialiseDevice(deviceData))
        } catch (err) {
    ...
    if (this.discoveryRun % 3 === 2) {
      ...
      setTimeout(() => {
        const names = Object.values(devicesToConnect)
        if (names.length > 0) {
          this.log.warn('%s: [%s].', this.lang.awaiting, names.join('], ['))
        }
      }, 3000)
    }
  ...
  async initialiseDevice (device) {
    try {
      ...
      deviceClients[device.UDN].on('error', err => {
        ...
        devicesToConnect[accessory.UUID] = accessory.displayName
        if (this.deviceConf[device.serialNumber]) {
          serialsToConnect.push(device.serialNumber)
        }
