...
export default class {
  ...
  async requestDeviceUpdate() {
    try {
      ...
      if (platformFuncs.hasProperty(data, 'brightness')) {
        this.receiveDeviceUpdate({
          name: 'Brightness',
          value: parseInt(data.brightness, 10),
        });
      }
    } catch (err) {
      if (this.enableDebugLogging) {
  ...
  async getCurrentBrightness() {
    ...
    return parseInt(data.brightness, 10);
  }
  ...
  async internalStateUpdate(value) {
    try {
      ...
      try {
        ...
        const eText = platformFuncs.parseError(err);
        this.log.warn('[%s] %s.', this.name, platformLang.brightnessFail, eText);
      }
    } catch (err) {
  ...
  async externalSwitchUpdate(value) {
    try {
      ...
      try {
        ...
        const eText = platformFuncs.parseError(err);
        this.log.warn('[%s] %s %s.', this.name, platformLang.brightnessFail, eText);
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err);
