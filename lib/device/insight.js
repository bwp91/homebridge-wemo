...
export default class {
  constructor(platform, accessory) {
    ...
    if (!platformFuncs.hasProperty(this.accessory.context, 'cacheLastWM')) {
      this.accessory.context.cacheLastWM = 0;
    }
    if (!platformFuncs.hasProperty(this.accessory.context, 'cacheLastTC')) {
      this.accessory.context.cacheLastTC = 0;
    }
    if (!platformFuncs.hasProperty(this.accessory.context, 'cacheTotalTC')) {
  ...
  async internalStateUpdate(value) {
    try {
      ...
      if (!value) {
        ...
        // Log the change if appropriate
        if (this.enableLogging) {
          this.log('[%s] %s [no].', this.name, this.lang.curOIU);
          this.log('[%s] %s [0W].', this.name, this.lang.curCons);
        }
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err, [this.lang.timeout, this.lang.timeoutUnreach]);
  ...
  externalInUseUpdate(value) {
    try {
      ...
      // Log the change if appropriate
      if (this.enableLogging) {
        this.log('[%s] %s [%s].', this.name, this.lang.curOIU, value ? 'yes' : 'no');
      }
    } catch (err) {
      // Catch any errors
      const eText = platformFuncs.parseError(err);
