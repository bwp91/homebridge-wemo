...
export default class {
  ...
  async requestDeviceUpdate() {
    try {
      ...
      // Parse the response
      const decoded = platformFuncs.decodeXML(data.attributeList);
  ...
  async internalIonizerUpdate(value) {
    ...
    try {
      ...
      if (this.enableLogging) {
        this.log('[%s] %s [%s].', this.name, platformLang.curIon, value ? 'on' : 'off');
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err, [platformLang.timeout, platformLang.timeoutUnreach]);
      this.log.warn('[%s] %s %s.', this.name, platformLang.cantCtl, eText);
  ...
  externalAirQualityUpdate(value) {
    try {
      ...
      // Log the change if appropriate
      if (this.enableLogging) {
        this.log('[%s] %s [%s].', this.name, platformLang.curAir, this.aqLabels[newValue]);
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err);
      this.log.warn('[%s] %s %s.', this.name, platformLang.cantUpd, eText);
  ...
  externalIonizerUpdate(value) {
    try {
      ...
      if (this.enableLogging) {
        this.log('[%s] %s [%s].', this.name, platformLang.curIon, value === 1 ? 'on' : 'off');
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err);
      this.log.warn('[%s] %s %s.', this.name, platformLang.cantUpd, eText);
  ...
  externalFilterLifeUpdate(value) {
    try {
      ...
      if (this.enableLogging) {
        this.log('[%s] %s [%s%].', this.name, platformLang.curFilter, value);
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err);
      this.log.warn('[%s] %s %s.', this.name, platformLang.cantUpd, eText);
