...
export default class {
  ...
  async internalTargetHumidityUpdate(value) {
    ...
    try {
      // Avoid multiple updates in quick succession
      const updateKeyHumi = platformFuncs.generateRandomString(5);
      ...
      if (this.enableLogging) {
        this.log('[%s] %s [%s%].', this.name, platformLang.tarHumi, newValue);
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err, [platformLang.timeout, platformLang.timeoutUnreach]);
      this.log.warn('[%s] %s %s.', this.name, platformLang.cantCtl, eText);
  ...
  externalTargetHumidityUpdate(value) {
    try {
      ...
      // Log the change if appropriate
      if (this.enableLogging) {
        this.log('[%s] %s [%s%].', this.name, platformLang.tarHumi, value);
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err);
      this.log.warn('[%s] %s %s.', this.name, platformLang.cantUpd, eText);
  ...
  externalCurrentHumidityUpdate(value) {
    try {
      ...
      // Log the change if appropriate
      if (this.enableLogging) {
        this.log('[%s] %s [%s%].', this.name, platformLang.curHumi, value);
      }
    } catch (err) {
      const eText = platformFuncs.parseError(err);
      this.log.warn('[%s] %s %s.', this.name, platformLang.cantUpd, eText);
