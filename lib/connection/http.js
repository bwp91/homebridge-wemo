  ...
  async sendDeviceUpdate (accessory, serviceType, action, body) {
    try {
      ...
      if (['at Object.<anonymous>', 'EHOSTUNREACH'].some(el => eText.includes(el))) {
        ...
        throw new Error(
          eText.includes('EHOSTUNREACH') ? this.lang.timeoutUnreach : this.lang.timeout
        )
      }
      throw err
    }
  ...
  async receiveDeviceUpdate (accessory, body) {
    try {
        ...
        switch (prop) {
          case 'BinaryState':
            try {
              ...
              const eText = this.funcs.parseError(err)
              this.log.warn('[%s] %s %s %s.', accessory.displayName, prop, this.lang.proEr, eText)
            }
            break
          case 'Brightness':
            try {
      ...
      if (accessory.context.enableLogging) {
        const eText = this.funcs.parseError(err)
        this.log.warn('[%s] %s %s.', accessory.displayName, this.lang.incFail, eText)
      }
    }
  }
}
